...
function bindEvents() {
  // Header Buttons
  document
    .getElementById('btnNeuesProjekt')
    .addEventListener('click', () => openProjektModal());
  document
    .getElementById('btnZeitEintragen')
    .addEventListener('click', openZeitModal);

  // Projekt Modal
  document
    .getElementById('closeProjekt')
    .addEventListener('click', closeProjektModal);
  document
    .getElementById('cancelProjekt')
    .addEventListener('click', closeProjektModal);
  document.getElementById('saveProjekt').addEventListener('click', saveProjekt);

  // Zeit Modal
  document
    .getElementById('closeZeit')
    .addEventListener('click', closeZeitModal);
  document
    .getElementById('cancelZeit')
    .addEventListener('click', closeZeitModal);
...
function renderProjekte(data) {
  ...
  if (!data || data.length === 0) {
    liste.innerHTML =
      '<div class="empty">Noch keine Projekte. Erstelle dein erstes Projekt!</div>';
    return;
  }

  liste.innerHTML = '';
  data.forEach((projekt) => {
    const totalStunden = (projekt.Zeiten || [])
...
function renderZeitenTabelle(zeiten, projektId) {
  ...
  const sorted = [...zeiten].sort(
    (a, b) => new Date(b.Start_Zeit) - new Date(a.Start_Zeit)
  );
...
function openProjektModal(projekt = null) {
  ...
  document.getElementById('projektBeschreibung').value = projekt
    ? projekt.Beschreibung || ''
    : '';
  document.getElementById('modalProjektTitel').textContent = projekt
    ? 'Projekt bearbeiten'
    : 'Neues Projekt';
...
async function saveProjekt() {
  ...
  const beschreibung = document
    .getElementById('projektBeschreibung')
    .value.trim();
...
window.deleteProjektConfirm = function (id, name) {
  document.getElementById('deleteMessage').textContent =
    `Projekt "${name}" wirklich löschen? Alle Zeiteinträge werden ebenfalls gelöscht.`;
...
async function saveZeit() {
  ...
  if (!projektId) {
    showToast('Bitte Projekt wählen', 'error');
    return;
  }
  if (!start) {
    showToast('Bitte Datum & Startzeit angeben', 'error');
    return;
  }
  if (!stunden || stunden <= 0) {
    showToast('Bitte gültige Stunden angeben', 'error');
    return;
  }
